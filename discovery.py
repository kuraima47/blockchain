...
class Address(object):
    ...
    def __init__(self, ip, udp_port, tcp_port=0, from_binary=False):
        ...
        try:
            ...
            _ip = str_to_bytes(ip)
            _ip = (
                (bytes_to_str(ip) if PY3 else unicode(ip))
                if ip_pattern.match(_ip)
                else _ip
            )
...
class Node(kademlia.Node):
    ...
    def to_uri(self):
        return utils.host_port_pubkey_to_uri(
...
class DiscoveryProtocol(kademlia.WireInterface):
    ...
    encoders = dict(cmd_id=chr, expiration=rlp.sedes.big_endian_int.serialize)

    decoders = dict(cmd_id=safe_ord, expiration=rlp.sedes.big_endian_int.deserialize)
...
class NodeDiscovery(BaseService, DiscoveryProtocolTransport):
    ...
    def send(self, address, message):
        ...
        try:
            ...
            log.debug(
                "udp write error", address=address, errno=e.errno, reason=e.strerror
            )
    ...
    def _handle_packet(self, message, ip_port):
        try:
            ...
            log.debug(
                "failed to handle discovery packet",
                error=e,
                message=message,
                ip_port=ip_port,
            )
    ...
    def start(self):
        ...
        port = self.app.config["discovery"]["listen_port"]
        # nat port mappin
        self.nat_upnp = add_portmap(port, "UDP", "DEVP2P Discovery")
        ...
        # bootstap
        if self.app.config["discovery"]["bootstrap_nodes"]:
            nodes = [
                Node.from_uri(x)
                for x in self.app.config["discovery"]["bootstrap_nodes"].split(" ")
            ]
            if nodes:
    ...
    def stop(self):
        ...
        remove_portmap(self.app.config["discovery"]["listen_port"], "UDP")
        if self.server:
            self.server.stop()
