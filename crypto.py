    ...
    for p in _openssl_lib_paths:
        if os.path.exists(p):
            ...
            if CIPHERNAMES.issubset(set(pyelliptic.Cipher.get_all_cipher())):
                break
if "pyelliptic" not in dir() or not CIPHERNAMES.issubset(
...
from utils import str_to_bytes, safe_ord, ascii_chr
...
class ECCx:
    def __init__(self, raw_pubkey=None, raw_privkey=None, curve="secp256k1"):
        if raw_privkey:
        ...
        if raw_pubkey:
            ...
            self.curve = (
                ec.SECP256K1() if curve == "secp256k1" else None
            )  # Vous pouvez ajouter d'autres courbes
            self.private_key = ec.generate_private_key(self.curve)
            self.public_key = self.private_key.public_key()

    def _kdf(self, shared_secret, length=32):
        hkdf = HKDF(
    ...
    def is_valid_key(self, raw_pubkey, raw_privkey=None):
        try:
            assert len(raw_pubkey) == 64
            failed = bool(
                self.raw_check_key(raw_privkey, raw_pubkey[:32], raw_pubkey[32:])
            )
    ...
    def ecies_decrypt(self, data, shared_mac_data=b""):
        ...
        # 2) verify tag
        if not pyelliptic.equals(
            hmac_sha256(key_mac, data[1 + 64 : -32] + shared_mac_data), tag
        ):
...
def _decode_sig(sig):
    return (
        safe_ord(sig[64]) + 27,
        bitcoin.decode(sig[0:32], 256),
        bitcoin.decode(sig[32:64], 256),
    )
