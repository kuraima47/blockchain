...
class Peer(gevent.Greenlet):
    ...
    def __init__(self, peermanager, connection, remote_pubkey=None):
        ...
        self.mux = MultiplexedSession(
            privkey, hello_packet, remote_pubkey=remote_pubkey
        )
        ...
        self.greenlets["dumb_checker"] = gevent.spawn_later(
    ...
    def receive_hello(
        ...
        for service in sorted(
            self.peermanager.wired_services, key=operator.attrgetter("name")
        ):
    ...
    def capabilities(self):
        return [
            (s.wire_protocol.name, s.wire_protocol.version)
        ...
        if self.offset_based_dispatch:
            for i, protocol in enumerate(self.protocols.values()):
                if packet.protocol_id == protocol.protocol_id:
                    break
                packet.cmd_id += (
                    0 if protocol.max_cmd_id == 0 else protocol.max_cmd_id + 1
                )
    ...
    def send(self, data):
        ...
        try:
            self.connection.sendall(
                data
            )  # check if gevent chunkes and switches contexts
