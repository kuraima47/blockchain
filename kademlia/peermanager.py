...
class PeerManager(WiredService):
    ...
    def __init__(self, app):
        ...
        self.peers = []
        self.errors = (
            PeerErrors() if self.config["log_disconnects"] else PeerErrorsBase()
        )
    ...
    def broadcast(
        ...
        peers_with_proto = [
            p for p in self.peers if protocol in p.protocols and p not in exclude_peers
        ]

        if not peers_with_proto:
            log.debug(
                "no peers with proto found", protos=[p.protocols for p in self.peers]
            )
        num_peers = num_peers or len(peers_with_proto)
        for peer in random.sample(
            peers_with_proto, min(num_peers, len(peers_with_proto))
        ):
    ...
    def connect(self, address, remote_pubkey):
        ...
        try:
            ...
            log.debug(
                "connection timeout", address=address, timeout=self.connect_timeout
            )
    ...
    def start(self):
        ...
        self.nat_upnp = add_portmap(
            self.config["p2p"]["listen_port"], "TCP", "DEVP2P Peermanager"
        )
        ...
        gevent.spawn_later(
            0.001, self._bootstrap, self.config["p2p"]["bootstrap_nodes"]
        )
    ...
    def stop(self):
        ...
        remove_portmap(self.config["p2p"]["listen_port"], "TCP")
        self.server.stop()
