...
k_bucket_size = 16
k_request_timeout = 3 * 300 / 1000.0  # timeout of message round trips
...
class FindNodeTask(object):
    ...
    def __init__(
        self, proto, targetid, via_node=None, timeout=k_request_timeout, callback=None
    ):
    ...
    def update(self, node, pingid=None):
        ...
        if pingid and (pingid not in self._expected_pongs):
            ...
            log.debug(
                "surprising pong",
                remoteid=node,
                expected=_expected_pongs(),
                pingid=encode_hex(pingid)[:8],
            )
            if pingid in self._deleted_pingids:
                ...
                for key in self._expected_pongs:
                    if key.endswith(node.pubkey):
                        log.debug(
                            "waiting for ping from node, but echo mismatch",
                            node=node,
        ...
        for _pingid, (timeout, _node, replacement) in list(
            self._expected_pongs.items()
        ):
            if time.time() > timeout:
                log.debug(
                    "deleting timedout node",
                    remoteid=_node,
        ...
        if pingid in self._expected_pongs:
            ...
            if replacement:
                ...
                self.routing.bucket_by_node(replacement).replacement_cache.append(
                    replacement
                )
        ...
        log.debug(
            "updated",
            num_nodes=len(self.routing),
            num_buckets=len(self.routing.buckets),
        )
            ...
            if time.time() < timeout:
                ...
                for close_node in closest[:k_find_concurrency]:
                    if not closest_known or close_node.id_distance(
                        nodeid
                    ) < closest_known.id_distance(nodeid):
                        log.debug(
                            "forwarding find request",
                            closest=close_node,
