...
class RLPxSession(object):
    ...
    def decrypt_header(self, data):
        ...
        expected_header_mac = mac(sxor(self.mac_enc(mac()[:16]), header_ciphertext))[
            :16
        ]
    ...
    def decode_authentication(self, ciphertext):
        ...
        try:
            (size, sig, initiator_pubkey, nonce, version) = self.decode_auth_plain(
                ciphertext
            )
        except AuthenticationError:
            (size, sig, initiator_pubkey, nonce, version) = self.decode_auth_eip8(
                ciphertext
            )
    ...
    def decode_auth_eip8(self, ciphertext):
        ...
        try:
            message = self.ecc.ecies_decrypt(
                ciphertext[2:size], shared_mac_data=ciphertext[:2]
            )
        except RuntimeError as e:
            raise AuthenticationError(e)
    ...
    def create_auth_ack_message(
        ...
        if eip8 or self.got_eip8_auth:
            msg = self.create_eip8_auth_ack_message(
                ephemeral_pubkey, self.responder_nonce, version
            )
            assert len(msg) > 97
        else:
    ...
    def encrypt_auth_ack_message(self, ack_message, eip8=False, remote_pubkey=None):
        ...
        if eip8 or self.got_eip8_auth:
            ...
            prefix = struct.pack(
                ">H", len(ack_message) + self.ecc.ecies_encrypt_overhead_length
            )
            self.auth_ack = self.ecc.ecies_encrypt(
                ack_message, remote_pubkey, shared_mac_data=prefix
            )
