...
class ConnectionMonitor(gevent.Greenlet):
    ...
    ping_interval = 15.0
    response_delay_threshold = 120.0  # FIXME, apply msg takes too long
    max_samples = 1000
    ...
    def latency(self, num_samples=max_samples):
        ...
        return (
            sum(self.samples[i] for i in range(num_samples)) / num_samples
            if num_samples
            else 1
        )
...
class P2PProtocol(BaseProtocol):
    ...
    class disconnect(BaseProtocol.command):
        ...
        class reason(object):
            ...
            bad_protocol = (
                2  # e.g. a malformed message, bad RLP, incorrect magic number
            )
            useless_peer = 3
            too_many_peers = 4
            already_connected = 5
            ...
            client_quitting = 8
            unexpected_identity = (
                9  # i.e. a different identity to a previous connection or
            )
        ...
        def create(self, proto, reason=reason.client_quitting):
            ...
            log.debug(
                "send_disconnect", peer=proto.peer, reason=self.reason_name(reason)
            )
        ...
        def receive(self, proto, data):
            ...
            proto.peer.report_error(
                "disconnected %s" % self.reason_name(data["reason"])
            )
            proto.peer.stop()
