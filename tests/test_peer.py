...
def get_connected_apps():
    a_config = dict(
        p2p=dict(listen_host="127.0.0.1", listen_port=3005),
...
def test_big_transfer():
    ...
    a_protocol = a_app.services.peermanager.peers[0].protocols[
        devp2p.p2p_protocol.P2PProtocol
    ]
    b_protocol = b_app.services.peermanager.peers[0].protocols[
        devp2p.p2p_protocol.P2PProtocol
    ]
...
def connect_go():
    a_config = dict(
        p2p=dict(listen_host="127.0.0.1", listen_port=3010),
        node=dict(privkey_hex=encode_hex(crypto.sha3(b"a"))),
    )
